...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct ProjectNpmBinManifest {
    schema: u32,
    generation: String,
    platform: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub(crate) struct ProjectNpmBinManifestEntry {
    name: String,
    backend: String,
    target: String,
}
...
#[derive(Debug)]
pub(crate) struct LockedNpmGraph<'a> {
    lockfile: &'a str,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct NpmGraphIdentity {
    sha256: String,
...
mod project_bin;
mod validate;
// Re-export with pub use (not pub(crate)): each item keeps its own
// visibility, so pub fns like publish_project_bin_generation /
// validated_project_bin_dir stay reachable across crates (osdk-cli, activate)
// exactly as before the split, while pub(crate) helpers stay crate-internal.
pub use project_bin::*;
