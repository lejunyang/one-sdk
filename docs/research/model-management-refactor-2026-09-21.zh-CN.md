...
> **2026-09-21 当日修订（第二轮）。**
>
>  初稿写作时本机 ComfyUI Desktop 尚未完成首次启动向导，ComfyUI 一族结论全部来自上游源码离线复现。此后用户完成了安装，本轮在
>
> **真实安装**
>
> 上补做核实与端到端实跑，产生三处
...
> **§3.2 推翻**
>
> ：初稿说「用户在 Storage 面板点一下 Add Shared Directory」—— 那不是可程序化接口。真实接口是 
>
> `%APPDATA%\Comfy Desktop\settings.json`
>
>  的 
>
> `modelsDirs`
...
> 据它生成 
>
> `instance-model-paths\<id>.yaml`
>
> ，再以 
>
> `--extra-model-paths-config`
>
>  传给 ComfyUI。完整链路由运行中进程的命令行证实（§3.2.1）。
> **§3.2 修正**
>
> ：Desktop 版布局是
>
...
> `settings.json`
>
> 。Comfy Desktop 自带维护脚本原文写明「the launcher will overwrite settings.json/installations.json on its own save cycle」，本轮也实测到它在自己的保存周期重写 
>
> `installations.json`
>
>  并轮转 
...
> **§5.9 新增（源码推导 → 实跑验证）**
>
> ：用安装自带的 Python 3.13.12 启动真实 ComfyUI v0.34.0，osdk 渲染视图中三个类别的权重
>
> **全部出现在&#x20;**
>
> `/object_info`
...
> **&#x20;的下拉框数据里**
>
> ，并带「渲染错 / 类别不匹配 / 未重扫」三路可区分的控制项。
> **§5.8 升级**
>
> ：torch+CUDA 从「索引页存在该 wheel」升级为
>
> **本机实跑**
>
> ：
...
>
> （RTX 4080 Laptop，驱动 610.47）。
> **§7 重写**
>
> ：给出「源码版 vs Desktop 版」的
>
> **明确推荐**
>
> （初稿只并列三条路径，未下结论）。
> **2026-09-21 第三轮补查（§5.12 / §7.2.1，源码版实机）。**
>
>  针对「源码版 ComfyUI 能否纯配置接模型」，本轮 
>
> `git clone`
>
>  当前上游（commit 
...
> `b0f4b7b`
>
> ）、用独立临时 venv 直接 import ComfyUI 自己的 
>
> `folder_paths`
>
> /
...
>
>  的两种形状都成立 —— 多条 base_path 直连快照内部子目录（零渲染即可用，配 P0-1 稳定入口），或 osdk 渲染一个 ComfyUI 形状视图根、一条 base_path；② 文件系统层面 junction /symlink/ 逐文件 hardlink 摆进默认 
>
> `models/<类别>/`
>
>  三者都被发现；③ 
...
> `.osdk-*`
>
>  元数据在真实源码版下拉框中泄漏数为 0；④ 用 ComfyUI 自己的 mtime 缓存做出「渲染错 / 类别不匹配 / 未重扫」三路可区分对照。结论：能走通，推荐配置发现（产品化形态为渲染视图 + 一条 base_path），osdk 需补 
>
> `model view render/export`
>
...
>
>  读大小，连续八分钟报 0 MB，几乎让我写下「下载卡住」这一错误结论 —— 该文件当时已有 1.65 GB。
>
> **缺陷在测量工具里，不在被测对象里。**
...
| 项目       | 事实                                                                                                                                                      | 核查位置                                       |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| 核心模块     | `crates/osdk-core/src/model/`：`mod.rs`(ModelStore/SnapshotManifest/snapshot\_key)、`pull.rs`、`env.rs`、`source.rs`、`provider/{huggingface,modelscope}.rs` | 文件列表，7 个文件共 85,463 B                       |
...
```
SNAPSHOT\_COUNT\_AFTER\_FIRST\_PULL=1

SNAPSHOT=4d44c30c20f5efa903ebe9b0

CURRENT\_JSON\_BODY={ "snapshot": "4d44c30c20f5efa903ebe9b0" }

REPARSE\_POINTS\_IN\_MODEL\_ROOT=0

SNAPSHOT\_COUNT\_AFTER\_SECOND\_PULL=2

CURRENT\_JSON\_BODY\_2={ "snapshot": "5be8be92c71b9618dd17a615" }
```
...
DOWNLOAD\_CACHE\_FILES=2 BYTES=1042966
...
```
HF\_ASSETS\_CACHE=\<cache>\pkg\models\huggingface\assets

HF\_ENDPOINT=https://huggingface.co

HF\_HOME=\<cache>\pkg\models\huggingface

HF\_HUB\_CACHE=\<cache>\pkg\models\huggingface\hub

HF\_XET\_CACHE=\<cache>\pkg\models\huggingface\xet
```
...
&#x20; \--feature-flag show\_signin\_button=true --feature-flag enable\_telemetry=true
...
\# When ComfyUI supports all\_model\_folders, this file will be simplified to:
...
\#     all\_model\_folders: true

comfy.desktop\_0:

&#x20; base\_path: 'E:\Comfy-Desktop\ComfyUI-Shared\models'
...
├── ComfyUI-Cache\download-cache\v0.34.0-env1\_win-nvidia\   # 独立环境包 .7z（2256.2 MB）

├── ComfyUI-Installs\ComfyUI\\

│   ├── ComfyUI\            # 源码树，含 .venv、models（26 子目录）、custom\_nodes
...
> Close the app before any mutating action — 
>
> **the launcher will overwrite settings.json / installations.json on its own save cycle.**



1. **本轮实测到了这个保存周期。** 端到端测试前后对 `%APPDATA%\Comfy Desktop` 取指纹：`installations.json` 的 SHA-256 变了，且 `installations.json.bak` 恰好变成了变更前的内容 —— 即**它自己轮转了备份**。diff 出的差异是纯增量字段 `lastLaunchedAt: 1789931393146`（= 03:09:53）与 `lastLaunchedAtByCategory`，与我的操作无关：Desktop 在安装完成后于 03:07:06 **自动启动了它自己的 ComfyUI**（进程命令行已捕获），这是它写的。

2. `.bak`**&#x20;只在第一次写。** 同一脚本注释说明 legacy 的 `installations.json.bak` / `settings.json.bak`「only written the first time, so the very first/original snapshot is preserved」—— 意味着 osdk 若写坏了 `settings.json`，用户的原始快照可能**已经被消耗掉**，无法回滚。
...
* `useChineseMirrors: true`：asar 内该设置的描述原文是「Git repositories clone from [gitcode.com](https://gitcode.com) instead of [github.com](https://github.com)」，tooltip 与字段描述均只提 **Git 与 PyPI**。它影响的是 custom node 的 clone 与 pip 安装，**不影响模型权重下载**（模型下载由用户在 ComfyUI 内部发起，走 HF/ModelScope）。所以与 osdk 的模型镜像**互补**：Desktop 管它自己的依赖获取，osdk 管模型权重获取。用户实际会同时得到两套，各管一段。
...
```
HF\_HOME = os.getenv("HF\_HOME", os.path.join(os.getenv("XDG\_CACHE\_HOME", \~/.cache), "huggingface"))

default\_cache\_path = os.path.join(HF\_HOME, "hub")

HF\_HUB\_CACHE = os.getenv("HF\_HUB\_CACHE", HUGGINGFACE\_HUB\_CACHE)   # 后者读 legacy HUGGINGFACE\_HUB\_CACHE

HF\_ASSETS\_CACHE = ... ; HF\_XET\_CACHE = os.getenv("HF\_XET\_CACHE", HF\_HOME/"xet")

ENDPOINT = os.getenv("HF\_ENDPOINT", "https://huggingface.co").rstrip("/")

HF\_HUB\_OFFLINE = \_is\_true(HF\_HUB\_OFFLINE or TRANSFORMERS\_OFFLINE)

HF\_HUB\_DISABLE\_SYMLINKS / HF\_HUB\_DISABLE\_SYMLINKS\_WARNING / HF\_HUB\_DISABLE\_IMPLICIT\_TOKEN

HF\_TOKEN\_PATH = HF\_HOME/"token"
```
...
```
\<HF\_HUB\_CACHE>/models--\<org>--\<name>/
...
│   ② 导出环境变量（HF\_HUB\_CACHE / LLAMA\_CACHE / MODELSCOPE\_…）│
...
PASS diffusion\_models sees exactly its own weight :: \["flux1-dev.safetensors"]

PASS vae sees exactly its own weight :: \["ae.safetensors"]

PASS text\_encoders sees exactly its own weight :: \["clip\_l.safetensors"]

PASS get\_full\_path resolves through the junction

PASS osdk metadata files stay out of the dropdowns :: \[]

PASS a single whole-snapshot mapping does lump categories together ::

&#x20;    \["text\_encoder\\\clip\_l.safetensors","unet\\\flux1-dev.safetensors","vae\\\ae.safetensors"]
...
PASS get\_full\_path after retarget finds the new file

PASS get\_full\_path after retarget reads the new snapshot :: b'weights-v2'
```
...
MUTATION one\_entry\_for\_whole\_snapshot exit=1 caught=True

MUTATION no\_retarget                  exit=1 caught=True

MUTATION metadata\_named\_as\_weights    exit=1 caught=True
...
```
huggingface\_hub 1.32.0

PASS client honours the HF\_HUB\_CACHE osdk exported

PASS try\_to\_load\_from\_cache finds a file osdk placed by commit

PASS a branch name resolves through refs/ that osdk wrote

PASS a nested path inside the repo resolves too

PASS snapshot\_download works fully offline against osdk's cache

PASS scan\_cache\_dir recognizes the repo osdk wrote :: \["osdk-probe/demo"]
...
```
MUTATION no\_refs\_file             exit=1 caught=True

MUTATION wrong\_repo\_dir\_name      exit=1 caught=True

MUTATION flat\_layout\_no\_snapshots exit=1 caught=True
...
> **稳定入口与跨卷视图用 junction（目录）+ hardlink（文件），不用符号链接。**
>
>  Unix 侧用 symlink（目录）+ hardlink（文件）。
>
> `LinkMode::Symlink`
>
>  的既有注释（
...
```
PASS diffusion\_models shows only the unet weight :: \["flux1-dev.safetensors"]

PASS vae shows only the vae weight :: \["ae.safetensors"]

PASS text\_encoders shows only the encoder weight :: \["clip\_l.safetensors"]

PASS checkpoints stays empty -- nothing was miscategorized :: \[]

PASS get\_full\_path resolves inside the view
...
```
MUTATION copy\_instead\_of\_hardlink     exit=1 caught=True

MUTATION flat\_view\_no\_type\_subfolders exit=1 caught=True
...
```
LEG1\_DOTNET\_NO\_PROXY\_ENV=ok ms=710 status=200
...
```
\--- probe\_timeout\_ms configured = 1500/5000/20000（三次）
...
```
error: unknown setting \`sources.probe\_timeout\_ms\`

&#x20; known: jobs, offline, yes, verify\_signatures, require\_checksums, attestations,

&#x20;        prerelease, link\_mode, lang, shims.include, shims.expose, shims.exclude,
...
```
METADATA\_MS=589 status=200

LARGEST\_FILE=rust\_model.ot SIZE=702517648

RANGE\_1MIB\_MS=1779 status=206 bytes=1048576
```
...
&#x20; MANIFEST\_ENDPOINT=https://hf-mirror.com
```
...
```
cu124: win\_amd64=46  python\_tags=cp38..cp313   sample: torch-2.6.0+cu124-cp312-cp312-win\_amd64.whl

cu126: win\_amd64=156 python\_tags=cp39..cp315   sample: torch-2.9.1+cu126-cp312-cp312-win\_amd64.whl

cu128: win\_amd64=92  python\_tags=cp39..cp314   sample: torch-2.9.1+cu128-cp312-cp312-win\_amd64.whl

cu129: win\_amd64=26  python\_tags=cp39..cp314   sample: torch-2.9.0+cu129-cp312-cp312-win\_amd64.whl

cu130: win\_amd64=110 python\_tags=cp39..cp315   sample: torch-2.9.1+cu130-cp312-cp312-win\_amd64.whl
```
...
```
PASS ComfyUI server started and answers /system\_stats

PASS CONTROL: the install's own models dir is readable by this server :: \["osdk-e2e-control.safetensors"]

PASS osdk view weight visible in UNETLoader.unet\_name  :: server=\["osdk-e2e-unet.safetensors"]  disk=\["osdk-e2e-unet.safetensors"]

PASS osdk view weight visible in VAELoader.vae\_name    :: server=\["osdk-e2e-vae.safetensors"]   disk=\["osdk-e2e-vae.safetensors"]

PASS osdk view weight visible in CLIPLoader.clip\_name  :: server=\["osdk-e2e-clip.safetensors"]  disk=\["osdk-e2e-clip.safetensors"]
...
PASS vae and text\_encoders resolve from the appended root
...
PASS without is\_default the osdk root is appended, not promoted :: index=4
...
```
MUTATION osdk\_root\_promoted\_to\_default    exit=1 caught=True

MUTATION only\_the\_filled\_category\_dirs    exit=0 EXPECTED-SURVIVE ok=True
...
> `registered[0]`
>
>  是 ComfyUI 内置根，两个文件都不在那儿。
>
> **失败的是我的断言，不是被测机制**
>
> 。改成「按顺序找第一个真正存在该文件的根」后通过，并额外加了一条把结论讲明白的断言（后追加的根不覆盖先前的）。这比直接改成硬编码的胜者要好：后者会在 
>
> `is_default`
>
>  改变行为时静默继续通过。
...
```
diffusion\_models = \["diffusion\_pytorch\_model.safetensors"]

vae               = \["vae.safetensors"]

text\_encoders     = \["encoder.safetensors"]

loras             = \["add-detail.safetensors", "style.safetensors"]   # 两个 repo 合并
```

即「多个 base\_path 段并存」「一 repo 多类别用多条类别映射直连快照子目录」**都可行，**
...
**形状 B——osdk 渲染一个 ComfyUI 形状的视图根，一条 base\_path 指过去。**
...
| 消费者配置复杂度    | 每模型每类别一行，模型一多就冗长             | 永远一条 base\_path             |
...
把链接直接放进 ComfyUI checkout 自带的 `models/`（不写 YAML，YAML 的 base\_path 就指向
...
```
models/diffusion\_models 作为 junction  -> 快照/unet   发现 diffusion\_pytorch\_model.safetensors

models/diffusion\_models 作为 symlink   -> 快照/unet   同样发现

models/diffusion\_models/\*.safetensors 逐文件 hardlink -> 快照字节   同样发现
...
3. **推荐配置发现，且产品形态推荐 B（渲染视图）+ 一条 base\_path**：零字节占用、快照保持
...
### 5.13 P1 呈现层的端到端总测（实现期，真实 CLI + 真实源码版 ComfyUI）

§5.12 用的是离线探针与手工构造；P1 呈现层（渲染引擎 + view CLI + `[models]` 声明）落地后，把整链做成了一个**可复跑、纳入&#x20;**`osdk task list`**&#x20;的集成测试**，而不是一次性手跑：



* 脚本：`scripts/test-comfyui-model-view.ps1`；配套无第三方依赖的环回 HF fixture `scripts/comfy-view-fixture-server.py` 与 headless 发现探针 `scripts/comfy-view-probe.py`。任务名 `comfyui-view-e2e`（`osdk.toml`，Windows only；需要一个 ComfyUI 检出 + 仅装了 PyYAML 的 Python，经 `-ComfyDir` / `OSDK_COMFYUI_DIR` 传入）。它**不启动 ComfyUI 服务、不需要 torch**：直接 import 真实源码树的 `folder_paths` + `utils.extra_config`（与 §5.12 同一加载路径）。

* **串起的完整链路（读产物判定，不看退出码）**：①环回真实 HTTP server 供三个小仓库 a/b/c，`osdk model pull` 产出**内容哈希快照**；②`osdk.toml` 里 a/b 写了 `[models.<name>.views.comfyui]`，pull 命中声明即**自动渲染**（无需 `model view add`），用渲染文件实际字节（AAA-UNET/AAA-VAE/AAA-TE）与 `.osdk-views.json` 归属记录确认；③`view export --to` 连跑两次：幂等（托管键块只一次）、保留用户 `user_block:` 段、**不含&#x20;**`is_default`；④真实 `folder_paths.get_filename_list`：三类都列出 `model.safetensors`、**checkpoints 反向保持空**，删渲染链接后再探类别恢复空 —— 第二反向控制保证「非空」确实来自渲染文件，能区分 §5.12.3 的三种空成因。

* **两视图根同名冲突（补 §9 旧第 10 条欠账）**：同一 profile=default 根下 c 也提供 `diffusion_models/model.safetensors` 时 `view add comfyui c` 被拒且在位者 a 链接字节不变；`remove --model c` 只拆 c、a 不动；重新 add a 后字节仍为 AAA。不同 profile（default/second）是两个互不相交的根，同名文件各自独立。另有跨平台 Rust 守护测试（不 cfg-gate）：`collision_leaves_the_first_models_link_intact_and_is_atomic_per_model`、`same_filename_across_models_is_a_reported_collision_not_an_overwrite`、`comfyui_shared_categories_survive_rebuild_and_model_removal`。

* **注入变红**：删掉渲染器 owner 冲突守卫后 `collision_leaves_…` 在「在位者链接不得被覆盖」处立即 FAILED，还原后绿；脚本自带两条反向控制（checkpoints 空、删链接后恢复空），让「下拉框非空即成功」的伪绿不成立；声明式自动渲染由「写了 views 声明再 pull、然后直接断言视图落盘字节」保证，声明若不触发渲染该字节断言就会红。

* **隔离边界**：全程临时 `OSDK_DATA/CACHE/CONFIG/STORE/INSTALL_DIR` 与临时 HOME，环回地址显式进 `NO_PROXY`；只用临时 ComfyUI 检出，**未触碰** `E:\Comfy-Desktop`、其 settings/installations、或真实 `E:\osdk-data`，结束清理。
...
```
LLAMA\_CACHE            → \<data>/views/gguf/\<profile>        # llama.cpp（已查证）

MODEL\_ENDPOINT         → 当前 provider 端点                   # llama.cpp 的镜像开关（已查证）

HF\_HUB\_CACHE           → \<data>/views/hf-cache/\<profile>     # 改为指向视图，而不是空缓存目录
```
...
include  = \["\*.safetensors", "\*.json", "text\_encoder\*/\*\*"]
...
map     = { "unet/" = "diffusion\_models", "vae/" = "vae", "text\_encoder/" = "text\_encoders" }
...
link\_mode   = "auto"        # auto|hardlink|copy；跨卷自动退化并报告

readonly    = true          # 视图文件设只读属性，防止就地写入污染 CAS

auto\_view   = true          # install 时自动建视图
```
...
&#x20;   pub requested\_revision: String,
...
&#x20;   \#\[serde(default, skip\_serializing\_if = "BTreeMap::is\_empty")]
...
const INSPECTED\_TABLES: &\[\&str] = &\["tools", "aliases", "settings", "tasks", "models"];

// collect\_requirements 的 match 增加：

"models" => collect\_models\_requirements(value, \&mut found),
```
...
&#x20;   pub default\_revision: &'static str,

&#x20;   pub default\_sources: fn() -> Vec\<Source>,

&#x20;   pub make: fn(allow\_auth: bool) -> Box\<dyn ModelProvider>,
...
pub static MODEL\_PROVIDERS: &\[\&ProviderSchema] = &\[\&HUGGINGFACE, \&MODELSCOPE, \&HTTP, \&LOCAL];
```
...
&#x20;     m.forward\_credentials = false; m },
...
1. `canonical_provider_endpoint` 因此天然把它折叠为官方 —— 无需改逻辑，只需该端点进入 `default_sources` 的返回值。**这一步要配一个会失败的测试**：删掉内置镜像条目后，「经镜像 pull 的 lock 仍记录官方端点」的测试要变红。

2. 修复 §5.6 的测速预算问题，否则新增镜像对排名毫无影响。

3. `MODEL_ENDPOINT`（llama.cpp）纳入 `model env` 导出。

4. **不做**：不引入 osdk 自己的模型代理 / 缓存服务器。与 Android SDK 那条约束同理（`android/mod.rs:11-16`）——osdk 直接从上游取，不再分发。
...
&#x20; "comfyui\_ref": "v0.34.0",  "comfyui\_commit": "12d5279438bfefc058a269eae805ceab6047777f",

&#x20; "python\_version": "3.13.12",

&#x20; "torch\_version": "2.12.1+cu130",

&#x20; "torchvision\_version": "0.27.1+cu130",

&#x20; "torchaudio\_version": "2.11.0+cu130",

&#x20; "uv\_version": "0.12.9",

&#x20; "vendor\_requirements\_content": "--extra-index-url https://download.pytorch.org/whl/cu130\ntorch==2.12.1+cu130\n…"
...
* Python 3.12（cp312）在 cu126/cu128/cu130 下都有 win\_amd64 wheel（实测）。
...
**第三轮（实现期）补了「声明→自动兑现」整链的端到端**（§5.13，`scripts/test-comfyui-model-view.ps1`，task `comfyui-view-e2e`）：真实 osdk CLI 对一个**临时 clone 的源码版 ComfyUI**（headless 加载真实 `folder_paths`，不需要 torch），跑通 `model pull`（环回真实 HTTP fixture，产出内容哈希快照）→ 命中 `osdk.toml` 的 `[models.<name>.views.comfyui]` 声明即自动渲染（无需手动 `view add`）→ `view export --to` 幂等合并且保留用户自己的 yaml 段、不带 `is_default` → 真实 `folder_paths.get_filename_list` 里 diffusion\_models/vae/text\_encoders 三类出现 `model.safetensors`、**checkpoints 反向保持空**、删掉渲染链接后类别恢复空（第二反向控制，用来区分 §5.12.3 的三种空成因）。整条链用临时 OSDK\_*/HOME 隔离，未碰 Desktop 与真实&#x20;*`E:\osdk-data`*。*
...
*10.&#x20;*~~没有测试多个 osdk 视图根同时挂载。~~*&#x20;**第三轮已验证（实现期端到端，见 §5.13 与&#x20;*`scripts/test-comfyui-model-view.ps1`*）**：①&#x20;**同一**视图根（profile=default）下两个模型提供同一消费者路径（*`diffusion_models/model.safetensors`*）时，第二个 render/*`view add`*&#x20;报*`already provided by model`*&#x20;且**在位者链接字节不被覆盖**；*`remove`*&#x20;一个模型只拆它自己的条目、另一个的链接与字节不动；rebuild 一个模型不改另一个。②&#x20;**不同**&#x20;profile（default vs second）是两个互不相交的视图根，同名文件各自存在、不冲突（脚本断言 second 根有独立的 vae/lora 字节）。守护测试在&#x20;*`view/mod.rs`*&#x20;的&#x20;*`collision_leaves_the_first_models_link_intact_…`*、*`same_filename_…`*、*`comfyui_shared_categories_survive_rebuild_and_model_removal`*，跨平台（Windows 与 WSL 原生 Linux 各跑一次，无 cfg 门控），并做过「移除 owner 冲突守卫→测试变红→还原」的注入验证。**仍未覆盖**的是「两个*不同 consumer\*（如 comfyui 与未来的 a1111）视图根被同一消费者同时挂载」—— 那要等 a1111 渲染器存在。
...
commands.rs:1171      \&app.ctx.dirs.installs  (scan\_installs\_for\_tool)

cargo\_package.rs:285  \&ctx.dirs.installs

github.rs:966         \&ctx.dirs.installs      (scan\_installs\_for\_tool)
...
**并保证跨人复现一致**。这与 npm\_package backend「驱真实 npm、不自己实现 npm」的
...
* pyproject + uv.lock：`uv sync`（官方语义即「按 lock 把项目环境装成一致状
  ...
  态」，见 [https://docs.astral.sh/uv/concepts/projects/sync/](https://docs.astral.sh/uv/concepts/projects/sync/) ；`uv run` 前会自动
...
* requirements.txt：`uv venv` + `uv pip install -r requirements.txt`；要冻结可
  ...
  （[https://docs.astral.sh/uv/pip/compile/](https://docs.astral.sh/uv/pip/compile/)）。
...
1. **回读 lock /receipt**：校验 `.venv` 的 `pyvenv.cfg`（creator、解释器，
...
[https://mise.jdx.dev/dev-tools/deps.html](https://mise.jdx.dev/dev-tools/deps.html) （2026-09 抓取）。
...
**可视化自检**：`check_visual.py` 对 `.visual.html` 做 19 项断言（锚点可达、唯一非空 `<title>`、8 类标签配平、根容器无 `100vh`/`height:100%`、**HTML 中引用的每个实测数字都能在本 md 中找到**、无占位数字、torch wheel 计数逐行比对），全部通过。其中「根容器高度」一项第一版写得过宽（把固定高度进度条内部的 `height:100%` 也算违规），收窄后另加了一条反向控制：向 `.fig` 注入 `height:100%` 必须被该检查捕获 —— 否则收窄就把它变成了一个什么都不测的检查。
...
* huggingface\_hub 常量：`https://raw.githubusercontent.com/huggingface/huggingface_hub/main/src/huggingface_hub/constants.py`
