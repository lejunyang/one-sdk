...
**安装必须附加 `--no-upgrade`，这不是优化而是必需项**（实机验证，winget 1.29.290）：

- 对**已安装**的包执行 `winget install`，winget 会打印「找到已安装的现有包。正在尝试升级已安装的包...」并**自动开始下载新版本**。实测在装有 Git 2.46.0 的机器上执行 `install --id Git.Git --exact`，它立即开始下载 2.55.0.3。
- 这意味着「确保某个包存在」这个意图在 winget 上会变成「顺手把它升级」——**一个用户没有请求的操作**，且可能破坏用户刻意停留的版本。
- 加上 `--no-upgrade` 后：不下载任何东西、版本保持不变、退出码 `0x8A150061`（`-1978335135`）。
- **因此 `0x8A150061` 必须归类为成功**：意图（包已存在）已经满足。若按「非零即失败」处理，幂等重跑会报错。
...
| --- | --- | --- |
| 安装成功 | `0` | 成功 |
| 包已安装（带 `--no-upgrade`） | `0x8A150061` / `-1978335135` | **成功**（幂等） |
| 找不到匹配的包 | `0x8A150014` / `-1978335212` | 失败：包 id 有误或源中不存在 |
| 找不到指定版本 | `0x8A150017` / `-1978335209` | 失败：如实报告该版本不可得，**不回退到其他版本** |
