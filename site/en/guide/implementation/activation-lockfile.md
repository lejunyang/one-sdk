...
`osdk activate <shell>` only prints shell code; the caller must `eval` or source it. Bash uses `PROMPT_COMMAND`, Zsh registers `precmd_functions`, Fish listens for `PWD` and `fish_prompt`, and PowerShell wraps the `prompt` function. All four are **prompt-level**: the hook runs at most once per rendered prompt, not several times per typed command. Every implementation invokes the hook immediately, without waiting for the first directory change. See [`commands::activate`](https://github.com/lejunyang/one-sdk/blob/main/crates/osdk-cli/src/commands.rs#L1847) and [`activation_script`](https://github.com/lejunyang/one-sdk/blob/main/crates/osdk-core/src/activate/mod.rs#L48).

The PowerShell wrapper preserves an existing `prompt`: activation captures whatever `prompt` is installed (falling back to the builtin default), calls it after its own hook, and `osdk deactivate powershell` puts it back. Oh My Posh, Starship, and hand-written prompts therefore survive activation. Earlier versions used `PostCommandLookupAction`, which fires on every *command lookup*, so a ten-iteration loop re-ran the whole activation 22 times; it is now the prompt-level hook described above.
