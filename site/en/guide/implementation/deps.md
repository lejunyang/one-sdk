...
## Listing is tiered; operand-free batch materialization is not
...
Materializing is a **correctness** problem, but two invocations must stay distinct.
An operand-free `osdk deps` is a batch operation and still covers every declared
root. An invocation with a bare provider name is targeted and may select only the
project root nearest the working directory; otherwise `osdk deps uv` crosses into
every same-named sub-project. Full addresses are exact: `//:uv` is the config root
and `//apps/api:uv` is the declared sub-project.

Accordingly, `wants_rooted` expands only for an operand-free batch run, `--all`,
`--filter`, or a rooted id with a non-empty path. Tests count `would run in` plans
and assert cwd rather than merely reading labels: a bare name from the root, a bare
name from a child, `//:provider`, and `//path:provider` must each produce exactly one
RunPlan.
