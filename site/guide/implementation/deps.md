...
兑现是**正确性**问题，但两种调用不能混为一谈：无操作数的 `osdk deps` 是全量批处理，
仍覆盖所有声明的 root；带裸 provider 名的调用是定点操作，只能命中当前工作目录最近的
那个 root。否则 `osdk deps uv` 会越界触发所有同名子项目。完整地址则精确命中：
`//:uv` 表示配置根，`//apps/api:uv` 表示声明的子项目。

所以 `wants_rooted` 只有在无操作数批处理、`--all`、`--filter` 或非空 rooted path 时展开。
测试不只数标签，而是数 `would run in` 并核对 cwd；根目录裸名、子目录裸名、`//:provider`
与 `//path:provider` 四种输入都必须各自产生且只产生一个 RunPlan。
