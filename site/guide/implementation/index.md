...
- osdk 的 BLAKE3 CAS 去重 SDK 与模型的已验证文件；原生 package cache 仍归各 manager
  管理。Cargo 工具在私有 staged Cargo home/target 中构建，Go 工具使用 osdk 控制的共享
  module/build cache；两者都在 CAS 外发布 binary。当前没有跨 manager 的 package tarball CAS。
