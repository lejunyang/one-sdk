...
精确受管 Node 版本与具体安装器会一起记录；已有 Node 工具项的其他选项会保留。声明 npm
工具本身不需要信任（安装默认传 `--ignore-scripts`），因此之后编辑 `[tools]`、新增包或
调整版本都不会要求重新 `osdk trust`；只有写入受管键才会。详见
[项目配置信任](./projects#哪些键需要信任)。
